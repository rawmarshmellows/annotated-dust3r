...
from src.annotated.decoder_block import TransformerDecoderBlock as AnnotatedDecoderBlock
...
def test_decoder_block_equivalence():
    ...
    with torch.no_grad():
        ...
        croco_state = croco_block.state_dict()
        annotated_state = annotated_block.state_dict()
        ...
        # Make sure all keys in annotated block are present in the new state dict
        for key in annotated_state.keys():
            if key not in new_state_dict:
                raise ValueError(f"Key {key} not found in new state dict")

    # Try loading state dict and capture any missing or unexpected keys
    incompatible_keys = annotated_block.load_state_dict(new_state_dict, strict=False)

    # Print and assert on any missing or unexpected keys
    ...
    ), f"There are missing keys in the state dict: {incompatible_keys.missing_keys}"
    assert (
        not incompatible_keys.unexpected_keys
    ), f"There are unexpected keys in the state dict: {incompatible_keys.unexpected_keys}"
    ...
    for name, (test_query, test_memory, test_query_pos, test_memory_pos) in test_cases.items():
        with torch.no_grad():
            ...
            assert max_diff_memory < 1e-6, f"Maximum memory difference exceeds tolerance for {name} input"

            print(f"Memory outputs match: {torch.allclose(croco_memory_out, annotated_memory_out, atol=1e-6)}")
            assert torch.allclose(
                croco_memory_out, annotated_memory_out, atol=1e-6
